package commands

import (
	"errors"

	"fmt"

	"github.com/solvent-io/zps/cli"
	"github.com/solvent-io/zps/zpm"
	"github.com/spf13/cobra"
)

type ZpmChannelCommand struct {
	*cobra.Command
	*cli.Ui
}

func NewZpmChannelCommand() *ZpmChannelCommand {
	cmd := &ZpmChannelCommand{}
	cmd.Command = &cobra.Command{}
	cmd.Ui = cli.NewUi()
	cmd.Use = "channel"
	cmd.Short = "Add a package to a channel with a ZPM repository"
	cmd.Long = "Add a package to a channel with a ZPM repository"
	cmd.PreRunE = cmd.setup
	cmd.RunE = cmd.run

	return cmd
}

func (z *ZpmChannelCommand) setup(cmd *cobra.Command, args []string) error {
	color, err := cmd.Flags().GetBool("no-color")

	z.NoColor(color)

	return err
}

func (z *ZpmChannelCommand) run(cmd *cobra.Command, args []string) error {
	image, _ := cmd.Flags().GetString("image")

	if cmd.Flags().Arg(0) == "" {
		return errors.New("Repo name required")
	}

	if cmd.Flags().Arg(1) == "" {
		return errors.New("Must specify a zpkg to add to a channel")
	}

	// Load manager
	mgr, err := zpm.NewManager(image)
	if err != nil {
		z.Fatal(err.Error())
	}

	mgr.On("channel", func(pkg string) {
		z.Info(fmt.Sprint("* channel set ", pkg))
	})

	err = mgr.Channel(cmd.Flags().Arg(0), cmd.Flags().Arg(1), cmd.Flags().Arg(2))
	if err != nil {
		z.Fatal(err.Error())
	}
	return nil
}
